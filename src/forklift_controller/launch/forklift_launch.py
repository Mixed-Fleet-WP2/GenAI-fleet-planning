...
def generate_launch_description():
    ...
    start_gazebo_server_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('ros_gz_sim'), 'launch',
                         'gz_sim.launch.py')),
        launch_arguments={'gz_args': ['-r -s ', world_proper]}.items())
        ...
        parameters=[
            {
                'config_file': os.path.join(
                    pkg_root, 'config', 'ros_gz_bridge.yaml'
                ),
                'use_sim_time': True,
            }
        ],
    ...
    spawn_robot_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_root, 'launch', 'spawn_tb4.launch.py')),
            #Sim time by default
        launch_arguments={'x_pose': '-8.0',
                          'y_pose': '0.0',
                          'z_pose': '0.0',
                          'roll': '0.0',
                          'pitch': '0.0',
    ...
    start_robot_state_publisher_cmd = Node(
        ...
        output='screen',
        parameters=[
            {'use_sim_time': True, 'robot_description': Command(['xacro', ' ', robot_urdf_file])}
        ]
    )
    ...
    rviz_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_root, 'launch', 'rviz_launch.py')
        ),
    ...
    set_env_vars_resources2 = AppendEnvironmentVariable(
            'GZ_SIM_RESOURCE_PATH',
            str(Path(os.path.join(pkg_root, 'meshes'))))
